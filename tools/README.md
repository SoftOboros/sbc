...
Offline committed-source indexing and CLI publication are implemented. Shared
query extraction, Django/MCP OAuth and dashboard integration remain incomplete.

The governing documents live in [docs/sbc-tools](../docs/sbc-tools/README.md).
The migration record preserves provenance and remaining authority-access work.
...
This package is developed in `SoftOboros/sbc` on `main`, without a release tag. First-party licensing is
...
`observations.check_observation` is a data-only single-repository comparison
component. A trusted host supplies captured corpus records, generated bytes and
the semantic validator. The result always has working-tree mode and null source/
